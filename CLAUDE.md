...
│   └── album/baby/     ← Trang Album Bé riêng (masonry + lightbox), xem mục bên dưới
...
## Trang Album Bé (`/album/baby`)

Ngoài trang chủ còn có một **trang album riêng** ở `public/album/baby/index.html` (file HTML độc lập, tự chứa CSS/JS bên trong — không liên quan tới `src/`).

- Bố cục **masonry** (ảnh so le) + **lightbox** (bấm ảnh phóng to).
- Ảnh nằm ở `public/album/baby/photos/` (hiện có `1.jpg`..`6.jpg`).
- Khi deploy, thư mục `public/` được copy nguyên vào `dist/`, nên trang này chạy tại địa chỉ `tên-miền/album/baby/`.
- **Thêm ảnh vào album:** chép ảnh (đã nén) vào `photos/`, rồi mở `index.html` của album thêm thẻ ảnh tương ứng.
- Muốn khách vào được album: thêm link `/album/baby/` vào menu trong `index.html` trang chủ (hiện chưa có link).
...
- [ ] Thêm link vào trang **Album Bé** (`/album/baby`) trong menu trang chủ.
